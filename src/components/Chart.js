...
const DonutChart = ({ data }) => {
  ...
  const chartLabels = data.map(item => {
    const percentage = totalVehicles ? ((item.value / totalVehicles) * 100).toFixed(1) : 0;
    ...
    datasets: [
      {
        ...
        backgroundColor: data.map(item => categoryColors[item.name] || '#000'),
  ...
  return (
    <div className="chart-container" style={{ maxWidth: '100%', height: '250px', margin: 0 }}>
