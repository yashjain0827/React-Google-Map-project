...
const Tracking = () => {
    ...
    useEffect(() => {
        const fetchDevices = async () => {
            try {
                setLoading(true);
                const response = await fetch("https://nkcfleet.nyggs.com/api/devices?all=true", {
                    headers: {
    ...
    //                         const timeDifference = (new Date(end.fixTime) - new Date(start.fixTime)) / 1000/ 60;
    ...
    //                 setTraveledPath((prevPath) => [...prevPath, currentPosition]);
    //                 setCurrentIndex(index);

    //                 setTripDetails((prevDetails) => ({
    //                     ...prevDetails,
    //                     stops : [...prevDetails.stops, {latitude: currentPosition.latitude, longitude: currentPosition.longitude }],
    ...
    useEffect(() => {
        if (positions.length > 1 && !isPaused) {
            ...
            const filteredPositions = positions.filter((pos, index, self) =>
                index === 0 || pos.latitude !== self[index - 1].latitude || pos.longitude !== self[index - 1].longitude
            );
            ...
            <Box sx={{ mt: 4, position: "relative" }}>
                {isLoaded ? (
                    <>
                       ...
                       <Button variant="contained" onClick={toggleDrawer(true)} sx={{position: "absolute", right:50, top:16,zIndex:10}}> Tracking Details </Button>
                       <Drawer
                            anchor="right"
                            open={drawerOpen}
                                ...
                                <>
                                    ...
                                    {infoBoxVisible && (
                                        ...
                                        <p>
                                            ...
                                            <br />
                                            Time: {new Date(currentPosition.fixTime).toLocaleTimeString()}
                                        </p>
                                    </div>
                                        </InfoWindow>
                                    )}
