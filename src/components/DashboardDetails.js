...
function DashboardDetails({
  ...
  const loadMoreData = () => {
    if (visibleData.length < showData.length) {
      const nextBatch = showData?.slice(
        visibleData.length,
        visibleData.length + ITEMS_PER_LOAD
      );
  ...
  return (
    <div className="dashboard">
      <div className="dashboard-buttons-container">
        ...
        <button
          className={`dashboard-button running ${
            activeCategory === "running" ? "selected" : ""
          }`}
        ...
        <button
          className={`dashboard-button stop ${
            activeCategory === "stop" ? "selected" : ""
          }`}
        ...
        <button
          className={`dashboard-button idle ${
            activeCategory === "idle" ? "selected" : ""
          }`}
        ...
        <button
          className={`dashboard-button offline ${
            activeCategory === "offline" ? "selected" : ""
          }`}
      ...
      <DonutChart
        data={chartData.filter(
          (item) =>
            activeCategory === "all" ||
            item.name.toLowerCase() === activeCategory
        )}
      />
        ...
        <div className="vehicle-grid">
          {visibleData?.map((vehicle, index) => (
            <div
              ...
              style={{ borderBottomColor: borderColorByStatus(vehicle.status) }}
            >
              <div style={{ height: "47px", width: "90px" }}>
                <img
                  src={iconByStatus(vehicle.status)}
                  alt="Truck Icon"
